---
name: mlflow
description: MLflow 3.14 (tracking + AI Gateway) on this stack. Load when logging experiments from Spark/Python jobs, registering models, or wiring AI Gateway routes to Anthropic / Ollama for LLM-in-pipeline patterns.
---

# MLflow 3.14

Tracking server: `http://localhost:5000`. AI Gateway: `http://localhost:5001`. Both in `docker-compose-mlflow.yml`. Backend: PostgreSQL (`mlflow` database). Artifact store: SeaweedFS (`s3://lakehouse/mlflow-artifacts`). Image base is `ghcr.io/mlflow/mlflow:v3.14.0-full` (the `-full` variant bundles `psycopg2-binary` + `boto3`, so Postgres and S3 work without a custom pip layer).
