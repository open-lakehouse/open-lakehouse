---
name: delta-ops
description: Delta Lake 4.3 operations on Spark 4.1 in this stack. Load when working with Delta tables specifically — covers OPTIMIZE, VACUUM, time travel, catalog-managed Delta, UniForm interop with Iceberg, and when to choose Delta over Iceberg here.
---

# Delta Lake operations

Delta Lake 4.3.1 is wired by default in this stack. The JARs (`delta-spark_2.13-4.3.1.jar`, `delta-storage-4.3.1.jar`) ship via `./lakehouse setup`, and `config/spark/spark-defaults.conf.example` enables both Iceberg and Delta extensions plus registers `spark_catalog` as the `DeltaCatalog`. You don't need per-session config — just write Delta. (4.3.1 specifically: 4.3.0 NPEs through the UC Spark connector — the stack pins 4.3.1 + the UC 0.5.x connector family, which together enable catalog-managed Delta.)
