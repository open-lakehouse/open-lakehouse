...
| **Proxy builds** — `feat/merge-offline` (Phase 6, rescoped 2026-08-23) | Add proxy-aware build args to the trunk Airflow and Jupyter Dockerfiles (T-6.2), including Airflow's Spark distribution URL; document `docs/deployment/proxy-builds.md`. Full air-gapped `setup --offline` vendoring (T-6.1/I-26) is deferred. | **shared:** Dockerfiles (`airflow`, `jupyter`, `jupyter-hosted`) and Compose files (`airflow`, `notebooks`) | loosely **#13** | Airflow and Jupyter build behind a proxy via invocation-time args; committed defaults stay public (`https://pypi.org/simple` and Apache archive), and the online path is unchanged |
