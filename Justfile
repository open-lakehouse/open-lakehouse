...
# Default release tag for image builds / deploys.
release_tag := env_var_or_default("SPARK_IMAGE_TAG", "v0.1.0")

# Show available recipes.
default:
    @just --list

# Deploy a stack to AWS. Usage: `just deploy spark-ecs [tag]`.
deploy target tag=release_tag:
    ...
    case "{{ target }}" in
      ...
      *)
        ...
        echo "Known targets: spark-ecs" >&2
        exit 1
        ;;
    esac
